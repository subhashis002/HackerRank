...
vector<int> closestNumbers(vector<int> arr) {
   vector<pair<int,int> > result;
    ...
    for(int i=0; i<arr.size();i++){
	    if(abs(arr[i]-arr[i+1])<max_difference){
		    max_difference = abs(arr[i]-arr[i+1]);
		    result.clear();
		    result.push_back(make_pair(arr[i],arr[i+1]));
	    }else if(abs(arr[i]-arr[i+1])==max_difference){
		    result.push_back(make_pair(arr[i],arr[i+1]));
	    }
    }
...
int main()
{
    ofstream fout("closestsumoutput.txt");
    ifstream fin("closestsuminput1.txt");
    if(!fin.is_open()){
	    cout<<"Input file cann't be open exited"<<endl;
	    exit(1);
    }
